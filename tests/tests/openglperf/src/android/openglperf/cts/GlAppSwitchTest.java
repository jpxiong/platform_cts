...
// two hidden classes for managing task switching
...
public class GlAppSwitchTest extends
    ...
    private void showOrHideReplicaIsland(boolean show) {
        try {
            // ActivityManager.moveTaskToFront API not available in GB, so use hidden API instead
            ActivityManagerNative.getDefault().moveTaskToFront(show ? mTaskIdReplica : mTaskIdSelf);
        } catch (RemoteException e) {
            // System dead, we will be dead too soon!
            Assert.fail("RemoteException from moveTaskToFront");
        }
    }
